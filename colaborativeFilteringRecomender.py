...
def createCFMatrix(unique_animes: int, unique_users: int, rating_df: DataFrame):
  ...
  print(anime_collaborative_filter) # each line represents an anime and each column represents all ratings given by a user to all animes (0 = no rating or actually 0)
...
def getUniqueIdsCount(anime_name_df: DataFrame, rating_df: DataFrame):
  ...
  unique_animes = {int(x): i for i, x in enumerate(anime_name_df.MAL_ID.unique())} # getting unique anime names from anime_name_df
...
def main():
  ...
  anime_collaborative_filter = createCFMatrix(unique_animes, unique_users, rating_df)
  ...
  while (want_to_search):
    ...
    result = getCFRecommendation(anime_collaborative_filter, anime_name_df, search_anime, n_neighbors = 10)
